...
BOT_TOKEN = "7673661564:AAGBThCopciYZOiN7XNpNJ2g1pDx_wUtqt0"
...
db = client["telegram_bot_new"]  # Using a new database name to avoid lock conflicts
users = db["users"]
orders = db["orders"]
...
user_states = {}  # Store user states for various operations
...
async def get_telethon_client_for_number(phone_number, session_string=None):
    ...
    try:
        ...
        # Create a new client using the session string or file
        if session_string:
            client = TelegramClient(StringSession(session_string), API_ID, API_HASH)
        else:
            # Create a new client using the file
...
# --- Command Handlers ---
@backup_bot.on_message(filters.command("start"))
async def start(client, message):
    ...
    # Create the menu with additional restart button
    menu_markup = button_menu()
    
    # Get the bot's username
    bot_info = await client.get_me()
    bot_username = bot_info.username
    
    # Create restart button that deep links back to the bot
    restart_button = InlineKeyboardButton(
        "🔄 Restart Bot", 
        url=f"https://t.me/{bot_username}?start=restart"
    )
    
    # Add the restart button to the menu
    menu_buttons = menu_markup.inline_keyboard
    menu_buttons.append([restart_button])
    updated_markup = InlineKeyboardMarkup(menu_buttons)
    
    # Send a welcome message with user's name
    try:
        await message.reply_text(
            f"**Hello {first_name}! 👋\nWelcome to the OTPGod Backup Bot!**\n\nChoose an option below:",
...
@backup_bot.on_message(filters.command("help"))
...
@backup_bot.on_message(filters.command("mynumbers") & filters.private)
...
@backup_bot.on_message(filters.command("id") & filters.private)
...
@backup_bot.on_callback_query(filters.regex("^main_menu$"))
...
@backup_bot.on_callback_query(filters.regex("^buy_number$"))
...
@backup_bot.on_callback_query(filters.regex("^buy_telegram$"))
...
@backup_bot.on_callback_query(filters.regex("^(ind|bd|usa|ng|other)_(regular|vip)$"))
async def handle_country_selection(client, callback_query):
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📋 Copy Binance ID", callback_data="copy_binance_id")],
...
@backup_bot.on_callback_query(filters.regex("^confirm_[a-z]+_(regular|vip)$"))
...
@backup_bot.on_message(filters.photo & filters.private)
...
@backup_bot.on_callback_query(filters.regex("^(approve|reject)_"))
...
@backup_bot.on_callback_query(filters.regex("^check_otp_"))
...
@backup_bot.on_callback_query(filters.regex("^login_success$"))
...
@backup_bot.on_callback_query(filters.regex("^support$"))
...
@backup_bot.on_callback_query(filters.regex("^recharge$"))
...
@backup_bot.on_callback_query(filters.regex("^check_balance$"))
...
@backup_bot.on_callback_query(filters.regex("^buy_whatsapp$"))
...
@backup_bot.on_message(filters.command("admin") & filters.private)
async def admin_panel(client, message):
    ...
    # First check with is_admin function
    is_admin_result = is_admin(user_id)
    print(f"is_admin check result: {is_admin_result}")
    
    # Then check direct comparisons for debugging
    direct_match = user_id == ADMIN_ID
    in_list = user_id in ADMIN_USER_IDS
    print(f"Direct ID match: {direct_match}, In admin list: {in_list}")
    
    if is_admin_result or direct_match:
        buttons = [
            ...
            [InlineKeyboardButton("📶 Monitor OTP", callback_data="admin_monitor_otp")],
            [InlineKeyboardButton("⚙️ Settings", callback_data="admin_settings")]
        ]
        keyboard = InlineKeyboardMarkup(buttons)
        await message.reply_text(
            f"👋 Welcome to the Admin Panel, {message.from_user.first_name}!\n\n"
            "Please select an option:",
            reply_markup=keyboard
        )
    else:
        await message.reply_text(f"⛔️ You are not authorized to use this command. Your user ID: {user_id}")
        return

@backup_bot.on_callback_query(filters.regex("^admin_manage_numbers$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_manage_orders$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_settings$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_reports$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_session_management$"))
async def admin_session_management(client, callback_query):
    """Show session management menu"""
    if str(callback_query.from_user.id) not in ADMIN_USER_IDS:
...
@backup_bot.on_callback_query(filters.regex("^admin_import_session$"))
...
@backup_bot.on_callback_query(filters.regex("^import_session_"))
...
@backup_bot.on_callback_query(filters.regex("^admin_export_session$"))
...
@backup_bot.on_callback_query(filters.regex("^export_session_"))
...
@backup_bot.on_callback_query(filters.regex("^admin_delete_session$"))
...
@backup_bot.on_callback_query(filters.regex("^delete_session_"))
...
def is_admin(user_id):
    ...
    result = user_id_str == ADMIN_ID or user_id_str in ADMIN_USER_IDS
    print(f"Admin check: user_id={user_id_str}, ADMIN_ID={ADMIN_ID}, ADMIN_USER_IDS={ADMIN_USER_IDS}, result={result}")
    return result

@backup_bot.on_callback_query(filters.regex("^session_login_number$"))
...
@backup_bot.on_callback_query(filters.regex("^session_import$"))
...
@backup_bot.on_callback_query(filters.regex("^session_export$"))
...
@backup_bot.on_callback_query(filters.regex("^session_delete$"))
...
@backup_bot.on_callback_query(filters.regex("^session_status$"))
...
@backup_bot.on_message(filters.private & filters.reply & filters.incoming)
async def handle_admin_input(client, message):
    ...
    if "temp_login" in user_data != None and user_data["temp_login"].get("step") == "awaiting_code":
    ...
    if "temp_add_number" in user_data != None and user_data["temp_add_number"].get("step") == "enter_phone":
...
@backup_bot.on_callback_query(filters.regex("^admin_back$"))
async def admin_back(client, callback_query):
    ...
    await safe_edit_message(
        callback_query.message,
        f"👋 Welcome back to the Admin Panel, {callback_query.from_user.first_name}!\n\n"
        "Please select an option:",
        reply_markup=keyboard
    )

@backup_bot.on_callback_query(filters.regex("^admin_add_number$"))
...
@backup_bot.on_callback_query(filters.regex("^add_number_type_(telegram|whatsapp)$"))
...
@backup_bot.on_callback_query(filters.regex("^add_number_(telegram|whatsapp)_(ind|bd|usa|ng|other)$"))
...
@backup_bot.on_callback_query(filters.regex("^add_number_(telegram|whatsapp)_(ind|bd|usa|ng|other)_(regular|vip)$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_view_numbers$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_pending_orders$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_sales_report$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_bulk_add$"))
...
@backup_bot.on_message(filters.command("bulkadd") & filters.private)
async def bulk_add_command(client, message):
    """Handle bulk addition of numbers"""
    user_id = message.from_user.id
    ...
    if len(command_parts) < 3:
        await message.reply_text(
            ...
            "/bulkadd country plan\n\n"
            "Example: /bulkadd in regular\n\n"
            "Valid countries: in, bd, usa, ng, other\n"
            "Valid plans: regular, vip"
        )
        return
    
    country_code = command_parts[1].lower()
    plan_type = command_parts[2].lower()
    
    # Validate country code
    valid_countries = ["in", "ind", "bd", "usa", "ng", "other"]
    if country_code not in valid_countries:
        await message.reply_text(
            "⚠️ Invalid country code. Please use one of:\n"
            "in, ind, bd, usa, ng, other"
        )
        return
    ...
    # Validate plan type
    if plan_type not in ["regular", "vip"]:
        await message.reply_text(
            "⚠️ Invalid plan type. Please use one of:\n"
            "regular, vip"
        )
        return
    
    # Store bulk add information in user state
    user_states[user_id] = {
        "action": "bulk_add",
        "country": country_code,
        "plan": plan_type,
        "step": "waiting_for_numbers"
    }
    
    # Prompt user for numbers
    await message.reply_text(
        f"📥 **Bulk Add: {country_code.upper()} {plan_type.capitalize()}**\n\n"
        f"Please send the list of phone numbers, one per line.\n\n"
        f"Example:\n"
        f"+917012345678\n"
        f"+917023456789\n"
        f"+917034567890\n\n"
        f"Each number should be in international format starting with '+'",
        reply_markup=InlineKeyboardMarkup([
...
@backup_bot.on_message(filters.command("exportstring") & filters.private)
...
@backup_bot.on_message(filters.command("loginnumber") & filters.private)
...
@backup_bot.on_message(filters.command("deletesession") & filters.private)
...
@backup_bot.on_message(filters.command("addsession") & filters.private)
...
@backup_bot.on_callback_query(filters.regex("^verify_success_(.+)$"))
...
@backup_bot.on_callback_query(filters.regex("^new_otp_(.+)$"))
...
@backup_bot.on_callback_query(filters.regex("^otp_help$"))
...
@backup_bot.on_message(filters.command("generateSession") & filters.private)
...
@backup_bot.on_message(filters.command("entercode") & filters.private)
...
@backup_bot.on_message(filters.command("enter2fa") & filters.private)
...
@backup_bot.on_message(filters.command("listnumbers") & filters.private)
...
@backup_bot.on_message(filters.command("clearinventory") & filters.private)
...
@backup_bot.on_callback_query(filters.regex("^confirm_clear_all$"))
...
@backup_bot.on_callback_query(filters.regex("^cancel_clear$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_clear_inventory$"))
async def admin_clear_inventory_button(client, callback_query):
    """Handle clear inventory button from admin panel"""
    user_id = callback_query.from_user.id
...
@backup_bot.on_message(filters.command("startmonitor") & filters.private)
...
@backup_bot.on_message(filters.command("stopmonitor") & filters.private)
...
@backup_bot.on_callback_query(filters.regex("^approve_payment_(.+)_(.+)$"))
...
@backup_bot.on_callback_query(filters.regex("^copy_upi_id$"))
...
@backup_bot.on_callback_query(filters.regex("^copy_binance_id$"))
async def handle_copy_binance_id(client, callback_query):
    """Send the Binance ID as a separate message for easy copying"""
    await callback_query.answer("Binance ID copied to clipboard!")
    await client.send_message(
        callback_query.from_user.id,
        f"`{BINANCE_ID}`\n\n_👆 Tap and hold to copy the Binance ID_"
    )

@backup_bot.on_message(filters.text & filters.private)
async def handle_text_input(client, message):
    ...
    try:
        ...
        if user_id in user_states:
            ...
            # Handle bulk add numbers
            elif state.get("action") == "bulk_add" and state.get("step") == "waiting_for_numbers":
                country_code = state.get("country")
                plan_type = state.get("plan")
                
                # Split text into lines and process each number
                numbers = text.strip().split('\n')
                if not numbers:
                    await message.reply_text("❌ No numbers provided. Please try again.")
                    return
                
                # Status message
                status_message = await message.reply_text("🔄 Processing numbers, please wait...")
                ...
                for number in numbers:
                    ...
                    if not re.match(r'^\+[1-9]\d{1,14}$', number):
                        invalid += 1
                        errors.append(f"❌ Invalid format: {number}")
                        continue
                    
                    # Check if number already exists
                    existing = numbers_inventory.find_one({"phone_number": number})
                    if existing:
                        skipped += 1
                        errors.append(f"⚠️ Already exists: {number}")
                        continue
                    ...
                    try:
                        ...
                        numbers_inventory.insert_one({
                            ...
                            "country": country_code,
                            "plan": plan_type,
                            "number_type": "telegram",  # Default to telegram
                            "status": "available",
                            "added_by": user_id,
                            "added_at": datetime.now(),
                            "session_string": None
                        ...
                        errors.append(f"❌ Error adding {number}: {str(e)}")
                
                # Clear user state
                del user_states[user_id]
                
                # Build result message
                result_message = f"📱 **Bulk Add Results**\n\n"
                result_message += f"Total numbers processed: {len(numbers)}\n"
                result_message += f"Added successfully: {added}\n"
                result_message += f"Skipped (already exist): {skipped}\n"
                result_message += f"Invalid format: {invalid}\n"
                
                if errors:
                    # Limit errors to first 10
                    error_display = errors[:10]
                    if len(errors) > 10:
                        error_display.append(f"... and {len(errors) - 10} more errors.")
            ...
            elif user_data != None and "temp_manual_monitor" in user_data != None and user_data["temp_manual_monitor"].get("step") == "waiting_for_number":
            ...
            elif user_data != None and "temp_manual_generate" in user_data != None and user_data["temp_manual_generate"].get("step") == "waiting_for_number":
                phone_number = text.strip()
            ...
            # Check if this message is an authentication code for session generation
            elif (user_id in user_states and user_states[user_id].get("action") == "phone_auth" and 
                  user_states[user_id].get("step") == "waiting_for_code") or (
                  user_data != None and "phone_auth" in user_data and user_data["phone_auth"].get("step") == "waiting_for_code"):
                
                # Get phone data from either source
                if user_id in user_states and user_states[user_id].get("action") == "phone_auth":
                    ...
                    print(f"Using auth data from user_states for {user_id}, phone {phone_number}")
                else:
                    phone_number = user_data["phone_auth"]["phone_number"]
                    phone_code_hash = user_data["phone_auth"]["phone_code_hash"]
                    print(f"Using auth data from database for {user_id}, phone {phone_number}")
                ...
                if not re.match(r'^\d{5,6}$', code):
                    await message.reply_text(
                        "❌ **Invalid code format**\n\n"
                        "The code should be 5-6 digits (e.g., 12345 or 123456).\n"
            ...
            # Check if this is a 2FA password for session generation
                  ...
                  user_states[user_id].get("step") == "waiting_for_2fa") or (
                  user_data != None and "phone_auth" in user_data and user_data["phone_auth"].get("step") == "waiting_for_2fa"):
                
                # Get phone data from either source
                if user_id in user_states and user_states[user_id].get("action") == "phone_auth":
                    phone_number = user_states[user_id].get("phone_number")
                    print(f"Using 2FA data from user_states for {user_id}, phone {phone_number}")
                else:
                    phone_number = user_data["phone_auth"]["phone_number"]
                    print(f"Using 2FA data from database for {user_id}, phone {phone_number}")
            ...
            elif user_data != None and "temp_add_number" in user_data != None and user_data["temp_add_number"].get("step") == "enter_phone":
...
@backup_bot.on_callback_query(filters.regex("^generate_session_"))
...
@backup_bot.on_callback_query(filters.regex("^begin_session_"))
...
@backup_bot.on_callback_query(filters.regex("^confirm_regenerate_"))
...
async def generate_session(client, message, phone_number, regenerate=False):
    ...
    try:
        ...
        users.update_one(
            ...
            upsert=True  # Add upsert to ensure the document is created if it doesn't exist
        )
        
        # Also store in user_states for redundancy
        user_states[user_id] = {
            "action": "phone_auth",
            "phone_number": phone_number,
            "phone_code_hash": phone_code_hash.phone_code_hash,
        ...
        print(f"Stored authentication data for user {user_id}, phone {phone_number}")
        
        # Ask the user to enter the code
        await status_message.edit_text(
            f"📱 Authentication code sent to {phone_number}\n\n"
            f"Please reply to this message with the code you received.\n\n"
            f"The code should be 5-6 digits (e.g., 12345 or 123456).",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Cancel", callback_data="admin_back")]
            ])
        )
...
@backup_bot.on_callback_query(filters.regex("^admin_generate_session$"))
...
@backup_bot.on_callback_query(filters.regex("^generate_session_manual$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_monitor_otp$"))
...
@backup_bot.on_callback_query(filters.regex("^monitor_otp_manual$"))
...
@backup_bot.on_callback_query(filters.regex("^start_monitor_"))
...
@backup_bot.on_callback_query(filters.regex("^confirm_monitor_"))
...
@backup_bot.on_callback_query(filters.regex("^stop_monitor_"))
...
@backup_bot.on_callback_query(filters.regex("^admin_search_numbers$"))
async def admin_search_numbers(client, callback_query):
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Back", callback_data="admin_manage_numbers")]
        ])
    )

@backup_bot.on_callback_query(filters.regex("^admin_inventory_stats$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_completed_orders$"))
async def admin_completed_orders(client, callback_query):
    ...
    if not completed_orders:
        await safe_edit_message(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 Back", callback_data="admin_manage_orders")]
            ])
        )
        return
...
@backup_bot.on_callback_query(filters.regex("^admin_revenue_stats$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_customer_list$"))
...
@backup_bot.on_callback_query(filters.regex("^admin_order_history$"))
...
if __name__ == "__main__":
    initialize_database()
    print("Griffin VIP Bot (Backup) is running...")
    ...
    try:
        print("Starting Griffin VIP Backup Bot...")
        backup_bot.run(run_bot())
        ...
        print("Griffin VIP Bot shutdown complete") 