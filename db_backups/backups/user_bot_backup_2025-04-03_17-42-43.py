...
async def handle_country_selection(client, callback_query):
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("UPI Payment", callback_data=f"payment_upi_{country_code}_{plan_type}")],
            [InlineKeyboardButton("Binance Pay", callback_data=f"payment_binance_{country_code}_{plan_type}")],
            [InlineKeyboardButton("🔙 Back", callback_data="buy_telegram")]
        ])
    )

@user_bot.on_callback_query(filters.regex("^payment_upi_(ind|bd|usa|ng|other)_(regular|vip)$"))
async def handle_upi_payment(client, callback_query):
    country_code = callback_query.data.split('_')[2]
    plan_type = callback_query.data.split('_')[3]
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("✅ I Have Paid", callback_data=f"confirm_upi_{country_code}_{plan_type}")],
            [InlineKeyboardButton("🔙 Back", callback_data=f"{country_code}_{plan_type}")]
        ])
    )

@user_bot.on_callback_query(filters.regex("^payment_binance_(ind|bd|usa|ng|other)_(regular|vip)$"))
async def handle_binance_payment(client, callback_query):
    country_code = callback_query.data.split('_')[2]
    plan_type = callback_query.data.split('_')[3]
    ...
    await safe_edit_message(
        callback_query.message,
        f"💳 Binance Payment Instructions\n\n"
        f"Country: {country_code.upper()}\n"
        f"Plan: {plan_type.capitalize()}\n"
        f"Amount: ₹{price}\n\n"
        f"Please send ₹{price} to Binance ID:\n"
        f"`{BINANCE_ID}`\n\n"
        "After payment, click 'I Have Paid' below.\n"
        "You'll need to provide a screenshot for verification.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📋 Copy Binance ID", callback_data="copy_binance_id")],
            [InlineKeyboardButton("✅ I Have Paid", callback_data=f"confirm_binance_{country_code}_{plan_type}")],
            [InlineKeyboardButton("🔙 Back", callback_data=f"{country_code}_{plan_type}")]
        ])
    )
...
async def handle_copy_binance_id(client, callback_query):
    await callback_query.answer(f"Copied: {BINANCE_ID}", show_alert=True)

@user_bot.on_callback_query(filters.regex("^confirm_upi_(ind|bd|usa|ng|other)_(regular|vip)$"))
async def handle_upi_payment_confirmation(client, callback_query):
    country_code = callback_query.data.split('_')[2]
    plan_type = callback_query.data.split('_')[3]
    ...
    await safe_edit_message(
        ...
        "Please send a clear screenshot of your UPI payment receipt.\n\n"
...
@user_bot.on_callback_query(filters.regex("^confirm_binance_(ind|bd|usa|ng|other)_(regular|vip)$"))
async def handle_binance_payment_confirmation(client, callback_query):
    country_code = callback_query.data.split('_')[2]
    plan_type = callback_query.data.split('_')[3]
    ...
    await safe_edit_message(
        callback_query.message,
        "📸 Payment Verification\n\n"
        "Please send a clear screenshot of your Binance payment receipt.\n\n"
        "Our admin will verify it within 15-30 minutes.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Back", callback_data=f"{country_code}_{plan_type}")]
        ])
...
async def handle_screenshot(client, message):
    ...
    try:
        ...
        # Get payment method
        payment_method = pending_payment.get("payment_method", "upi").capitalize()
        
        # Log important information
        logger.info(f"Processing payment screenshot from user {user_id}, payment ID: {payment_id_str}, method: {payment_method}")
        print(f"Payment screenshot received: user={user_id}, payment_id={payment_id_str}, method={payment_method}")
        ...
        try:
            ...
            await message.reply_text(
                ...
                f"Admin will verify your {payment_method.lower()} payment shortly."
            )
