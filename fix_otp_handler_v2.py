...
# The OTP handler code to be added - with fixed string literals
OTP_HANDLER = """
...
def restore_from_backup():
    """Restore the user_bot.py from backup if it exists"""
    if os.path.exists(BACKUP_FILE):
        logger.info(f"Restoring {USER_BOT_FILE} from {BACKUP_FILE}")
        shutil.copy2(BACKUP_FILE, USER_BOT_FILE)
        return True
    return False
